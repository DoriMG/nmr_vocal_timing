...
def inh_ramp_model(x, stim_len, tot_len=1800):
    A_exc = x[0]
    inh_factor = x[1]
    r_mod = x[2]
    ...
    data_folder = r'D:\Frankfurt\Papers\nmr_antiphonal\paper_code\fig4\data'
    bins_all = np.load(os.path.join(data_folder,'data_long_short_noise_perc.npy'))
    if stim_len == 200:
        bins_all = bins_all[:,0,:]
    elif stim_len == 1600:
        bins_all = bins_all[:,1,:]
    ...
    call_times = []
    stim = np.concat([np.ones((stim_len,)), np.zeros((tot_len-stim_len, ))])
    n_rep = int(3*60*1000/(tot_len))
...
def noise_inh_model(x,  stim_len, tot_len=1800):
    A_exc = x[0]
    r_mod = x[1]
...
def ramp_model(x,  stim_len, tot_len=1800):
    A_on = x[0]
    A_off = x[1]
    r_mod_on = x[2]
    r_mod_off = x[3]
...
# Load for KS test
load_folder = r'D:\Frankfurt\Papers\nmr_antiphonal\paper_code\fig2\data'
all_calls_200ms = np.load(os.path.join(load_folder,'all_calls_200ms.npy'))
all_calls_1600ms = np.load(os.path.join(load_folder,'all_calls_1600ms.npy'))


tot_len = 1800
noise_lens = [200, 1600]
n_ani = 20
for stim in noise_lens:
    twotier_call_times = []
    feedback_call_times = []
    single_ramp_call_times = []
    for i in range(n_ani):
        error, response, error_p_ani, call_times  = inh_ramp_model(x_feedback, stim, tot_len)
        ...
        error, response, error_p_ani, call_times  = ramp_model(x_two_tier, stim, tot_len)
        error_df = pd.DataFrame(
                {'data': [error],
                 'error_p_ani': [np.nanmean(error_p_ani)],
                 'error_min': [np.nanmin(error_p_ani)],
        ...
        error, response, error_p_ani, call_times  = noise_inh_model(x_noise,  stim, tot_len)
        error_df = pd.DataFrame(
                {'data': [error],
                 'error_p_ani': [np.nanmean(error_p_ani)],
                 'error_min': [np.nanmin(error_p_ani)],
    ...
    if stim==200:
        all_calls = all_calls_200ms
    elif stim==1600:
        all_calls = all_calls_1600ms
...
out_folder = r'D:\Frankfurt\Papers\nmr_antiphonal\paper_code\fig4\data'
all_error.to_csv(os.path.join(out_folder,'model_errors_long_short.csv'), index=False)  
all_model_response.to_csv(os.path.join(out_folder,'model_responses_long_short.csv'), index=False)  
